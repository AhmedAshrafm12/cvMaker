...
<ul>
<li><h4>adress <span><?php echo $row['adress']?></span></h4></li>
<li><h4>phone <span><?php echo $row['mobile']?></span></h4></li>
<li><h4>Email <span><?php echo $row['email']?></span></h4></li>
<li><h4>adress <span><?php echo $row['adress']?></span></h4></li>
</ul>
</div>
<div>
...
for($i=0;$i<count($cern);$i++){
    if(!empty($cern[$i])){
    echo"<div><h4><span>".$cert[$i]."</span>".$cern[$i]."</h3>";
    echo "</div>";
}}
?>
</div>
...
<div class='chcolor'>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>?uni=<?php echo $uni?>" method='POST' style='padding:10px;margin-top:30px'>
